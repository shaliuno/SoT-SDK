#pragma once

// SOT: Sea of Thieves (1.0) SDK

#ifdef _MSC_VER
	#pragma pack(push, 0x4)
#endif

#include "../SDK.hpp"

namespace SDK
{
//---------------------------------------------------------------------------
//Parameters
//---------------------------------------------------------------------------

// Function BP_CliffGenerator.BP_CliffGenerator_C.Draw_Debug_Numbers
struct ABP_CliffGenerator_C_Draw_Debug_Numbers_Params
{
	int                                                CurrentIndex;                                             // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0000
	int                                                NextIndex;                                                // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0004
	struct FVector                                     StartLocation;                                            // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0008
	struct FVector                                     EndLocation;                                              // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0014
	struct FVector                                     StartTangent;                                             // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0020
};

// Function BP_CliffGenerator.BP_CliffGenerator_C.Deform_Terrain
struct ABP_CliffGenerator_C_Deform_Terrain_Params
{
};

// Function BP_CliffGenerator.BP_CliffGenerator_C.Build_Element
struct ABP_CliffGenerator_C_Build_Element_Params
{
	int                                                Index;                                                    // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0000
	class UStaticMesh*                                 ElementMesh;                                              // (CPF_Parm, CPF_ZeroConstructor, CPF_IsPlainOldData) 0008
};

// Function BP_CliffGenerator.BP_CliffGenerator_C.UserConstructionScript
struct ABP_CliffGenerator_C_UserConstructionScript_Params
{
};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
